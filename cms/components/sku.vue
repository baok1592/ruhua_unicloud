		...
		<div class="form-group">
			...
			<div class="form-item" v-for="(attr,index) in attrs" :key="index">
				<div class="form-title">
					<el-input class="sku-name" v-model="attr.pName" size="small" placeholder="规格名" />
					<!-- 	<el-checkbox v-model="checked" v-if="index==0" size="small">添加规格图片</el-checkbox> -->
					<span class="delete" @click="toDelete(index)">×</span>
				</div>
				<ul class="form-list">
					<li v-for="(item,index2) in attr.spec" :key="index2">
						<div class="spgg_r_02_2">
							<el-input v-model="item.cName" size="small"></el-input>
					...
					<li>
						<div style="margin-left: 15px;">
							<el-button size="small" @click="add_canshu(index)" icon="el-icon-plus">添加</el-button>
						</div>
					</li>
				</ul>
			</div>
			...
			tableData: function() {
				...
				for (let i = 0; i < len; i++) {
					...
					for (let j = 0; j < len2; j++) {
						...
						let specId = attrs[i]["spec"][j]["specId"];	
						if (!cName) {
							continue;
						}
						++specLen;
			...
			tableList: function() {
				...
				for (let i = 0; i < rows; i++) {
					...
					for (let j = 0; j < srcData.length; j++) {
						...
						listItem["specId"] = spec[index]["specId"];
					}
					listItem["price"] = "";
					listItem["stock_num"] = "";
